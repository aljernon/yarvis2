...
*/5 * * * * root SIGNAL_DB_PATH=$SIGNAL_DB_PATH SIGNAL_CLI_PATH=$SIGNAL_CLI_PATH SIGNAL_CLI_CONFIG=$SIGNAL_CLI_CONFIG SIGNAL_ACCOUNT=$SIGNAL_ACCOUNT PYTHONUNBUFFERED=1 /opt/accumulator-venv/bin/python /opt/accumulator/capture.py >> $CRON_LOG 2>&1
EOF
