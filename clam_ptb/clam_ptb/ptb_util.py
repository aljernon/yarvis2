...
from clam_ptb.util import ensure, to_truncated_str
...
class InterruptionScope:
    ...
    def is_interrupted(self) -> bool:
        from clam_ptb.interruption_scope_internal import CHAT2LAST_MESSAGE_ID
...
def build_interruptable_scope(chat_id: int, message_id: int | None = None):
    from clam_ptb.interruption_scope_internal import INTERRUPTABLES
...
async def reply_maybe_markdown(
    ...
    *,
    ...
    if len(text_md) > 4000 and (final_update or message is None):
        ...
        date = datetime.datetime.now().date()
        with NamedTemporaryFile(
            mode="w", prefix=f"clam_message_{date}_{gettempprefix()}", suffix=".txt"
        ) as stream:
            stream.write(text)
            stream.flush()
            return await bot.send_document(chat_id=chat_id, document=stream.name)  # type: ignore
